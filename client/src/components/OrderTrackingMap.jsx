  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4 p-4 bg-gray-50 rounded-xl">
        <div className="flex items-center gap-3">
          <div className={`w-3 h-3 rounded-full ${
            trackingStatus?.status === "delivered" ? "bg-green-600" :
            trackingStatus?.status === "shipped" ? "bg-blue-600" :
            trackingStatus?.status === "processing" ? "bg-yellow-500" : "bg-red-600"
          }`} />
          <span className="font-medium text-gray-800 capitalize">{trackingStatus?.status}</span>
        </div>
          ...
          {trackingStatus?.timeline.map((item, index) => (
            <div key={index} className="relative flex gap-4">
              <div className={`relative z-10 w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 ${
                ...
                <item.icon size={20} className={item.completed ? "text-blue-600" : "text-gray-500"} />
              </div>
      ...
      <div className="bg-gray-50 rounded-xl p-4 text-center">
        <div className="h-48 bg-gradient-to-br from-blue-100 to-orange-100 rounded-lg flex items-center justify-center">
          <div className="text-center">
            <MapPin size={32} className="text-blue-600 mx-auto mb-2" />
            <p className="text-gray-500 text-sm">Live tracking map will appear here</p>
          </div>
        </div>
      </div>
    </div>
