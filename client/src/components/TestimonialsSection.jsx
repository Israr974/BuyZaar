    ...
    <div className="py-6 md:py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        {testimonials.map((review) => (
          ...
          >
            {/* Avatar and Name - Smaller on mobile */}
            <div className="flex items-center gap-3 mb-3">
              <div className={`${isMobile ? 'w-8 h-8 text-sm' : 'w-12 h-12 text-lg'} rounded-full bg-gradient-to-br from-blue-600 to-orange-500 flex items-center justify-center text-white font-bold`}>
            ...
            {/* Review Title */}
            {review.title && (
              <h5 className={`font-semibold text-gray-700 ${isMobile ? 'text-xs' : 'text-sm'} mb-1`}>
                {review.title}
              </h5>
            )}
            
            {/* Review Comment */}
            <p className={`text-gray-600 ${isMobile ? 'text-xs' : 'text-sm'} leading-relaxed line-clamp-4`}>
              {review.comment}
            </p>
            
            {/* Footer with date and helpful count */}
            <div className="flex items-center justify-between mt-3 pt-2 border-t border-gray-100">
              <span className={`${isMobile ? 'text-[9px]' : 'text-xs'} text-gray-400`}>
