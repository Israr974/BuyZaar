...
import {  
  Upload, Image as ImageIcon, CheckCircle, Save,
...
const UploadCategoryModel = ({ onClose, editData, onSuccess }) => {
  ...
  const validateForm = () => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      document.getElementById(`error-${firstError}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        toast.success(editData ? "Category updated successfully!" : "Category added successfully!");
        onSuccess?.();
        onClose();
      ...
      >
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <textarea
                ...
                disabled={isSaving}
                rows={3}
                className={`input w-full resize-none ${errors.description ? 'border-error' : ''}`}
              />
              ...
              {imagePreview ? (
                <div className="relative group">
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                    <button
                      ...
                      disabled={isSaving}
                      className="p-2.5 rounded-lg bg-white text-text hover:bg-primary hover:text-white transition-all hover:scale-110"
                    >
                      <Camera size={18} />
                    </button>
                    <button
                      ...
                      disabled={isSaving}
                      className="p-2.5 rounded-lg bg-red-500 text-white hover:bg-red-600 transition-all hover:scale-110"
                    >
                      <Trash2 size={18} />
                    </button>
                  </div>
                ...
                >
                  ...
                  <div className="text-center">
                    <p className="text-sm font-medium text-text">Click to upload image</p>
                    <p className="text-xs text-text-muted mt-1">PNG, JPG, GIF, WebP up to 2MB</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="sticky bottom-0 bg-card border-t border-border p-5">
          <div className="flex justify-end gap-3">
            <button onClick={onClose} disabled={isSaving} className="btn btn-outline px-5 py-2.5 rounded-xl">
              Cancel
            </button>
            <button
