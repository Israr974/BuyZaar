...
const Login = () => {
  ...
  const resendVerificationEmail = async (email) => {
    try {
      ...
      // Silent fail - error toast already shown
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (resData.requiresEmailVerification === true) {
          toast(
            (t) => (
              <div className="flex flex-col gap-2">
                ...
                <button
                  ...
                  className="mt-1 px-3 py-1.5 bg-blue-500 text-white rounded-lg text-sm hover:bg-blue-600 transition"
                >
                  Resend Verification Email
                </button>
              </div>
            ...
            { 
              duration: 8000, 
              position: "top-center",
              style: { background: "#1f2937", color: "#fff", padding: "16px" }
            }
          );
        }
      ...
      // Error handled by AxiosError, but we don't have it imported
